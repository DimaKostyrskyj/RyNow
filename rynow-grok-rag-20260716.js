...
$$("[data-soon]").forEach(button => {
  button.addEventListener("click", () => {
    showToast(`${button.dataset.soon}: функция скоро появится`);
  });
});
...
async function askForum(question) {
  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      throw new Error(
        `AI API вернул не JSON (${response.status}): ${shortText || "пустой ответ"}`
      );
    }

    if (!response.ok) {
      throw new Error(data.error || `Ошибка AI API: ${response.status}`);
    }
